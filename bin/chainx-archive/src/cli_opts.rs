...
pub struct CliOpts {
    ...
    pub chain_spec: chainx_cli::chain_spec::ChainSpec,
}
...
impl CliOpts {
    pub fn parse() -> Self {
        ...
        if spec.is_some() {
            match spec {
                Some("dev") => {
                    chain_spec = chainx_cli::chain_spec::development_config();
                }
                Some("") | Some("local") => {
                    chain_spec = chainx_cli::chain_spec::local_testnet_config();
                }
                path => {
                    chain_spec = chainx_cli::chain_spec::ChainSpec::from_json_file(
